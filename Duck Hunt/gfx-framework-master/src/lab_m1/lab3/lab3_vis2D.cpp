...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    */
    ...
    if (window->KeyHold(GLFW_KEY_W)) {
        logicSpace.y += 0.05f * deltaTime;
    }
    if (window->KeyHold(GLFW_KEY_S)) {
        logicSpace.y -= 0.05f * deltaTime;
    }
    if (window->KeyHold(GLFW_KEY_A)) {
        logicSpace.x -= 0.05f * deltaTime;
    }
    ...
    if (window->KeyHold(GLFW_KEY_Z)) {
        logicSpace.width -= 0.4f * deltaTime;
        logicSpace.height -= 0.4f * deltaTime;
        logicSpace.y += 0.2f * deltaTime;
        logicSpace.x += 0.2f * deltaTime;
    }
