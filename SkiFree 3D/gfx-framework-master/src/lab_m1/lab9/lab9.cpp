...
void Lab9::updateRotateAngle() {
    glm::vec3 forward = GetSceneCamera()->m_transform->GetLocalOZVector();
    forward = glm::normalize(glm::vec3(forward.x, 0, forward.z));
    rotateForGrass = atan2(forward.x, forward.z);
}
...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(4.0f, 0.5f, -2.0f));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
        modelMatrix = glm::rotate(modelMatrix, rotateForGrass, glm::vec3(0, 1, 0));
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    if (texture1)
    {
        ...
        // Nu mixam culoarea
        glUniform1i(glGetUniformLocation(shader->program, "toMix"), 0);
    }
    ...
    if (texture2)
    {
        ...
        // Trebuie mixata culoarea
        glUniform1i(glGetUniformLocation(shader->program, "toMix"), 1);
    }

    // Trimitem timpul
    int location = glGetUniformLocation(shader->program, "timeElapse");
    float timeElapsed = Engine::GetElapsedTime();
    glUniform1f(location, timeElapsed);

    // Trimitem daca e glob
    location = glGetUniformLocation(shader->program, "isGlobe");
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    GLenum texture_min = GL_NEAREST_MIPMAP_NEAREST;
    GLenum texture_mag = GL_NEAREST;
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, texture_min);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, texture_mag);
