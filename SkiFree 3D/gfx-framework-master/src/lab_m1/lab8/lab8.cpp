...
void Lab8::Init()
{
    ...
    // Light & material properties
        ...
        for (int i = 0; i < nrLights; i++) {
            point_light_pos[i] = glm::vec3(0, i + 1, i + 1);
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    // Trimitem spotty
    glUniform2i(glGetUniformLocation(shader->program, "is_spotty"), isSpotty[0], isSpotty[1]);
    // Trimitem cutoff
    glUniform2f(glGetUniformLocation(shader->program, "cut_off"), cutOff[0], cutOff[1]);
   
    // Pentru mai multe lumini
    GLuint location = glGetUniformLocation(shader->program, "point_light_pos");
    glUniform3fv(location, nrLights, glm::value_ptr(point_light_pos[0]));
    location = glGetUniformLocation(shader->program, "point_light_dir");
    glUniform3fv(location, nrLights, glm::value_ptr(point_light_dir[0]));
    
    //location = glGetUniformLocation(shader->program, "spotties");
    //glUniform2i(location, 0, 1);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_W)) point_light_pos[currentLight] -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_A)) point_light_pos[currentLight] -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_S)) point_light_pos[currentLight] += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_D)) point_light_pos[currentLight] += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_E)) point_light_pos[currentLight] += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_Q)) point_light_pos[currentLight] -= up * deltaTime * speed;

        // TODO(student): Set any other keys that you might need
        if (window->KeyHold(GLFW_KEY_G)) {
            cutOff[currentLight] = cutOff[currentLight] + deltaTime * 12.5f;
        }
        if (window->KeyHold(GLFW_KEY_H)) {
            cutOff[currentLight] = cutOff[currentLight] - deltaTime * 12.5f;
        }
        if (window->KeyHold(GLFW_KEY_I)) {
            angleX[currentLight] = angleX[currentLight] + deltaTime * speed;
        }
        if (window->KeyHold(GLFW_KEY_J)) {
            angleZ[currentLight] = angleZ[currentLight] + deltaTime * speed;
        }
        ...
        point_light_dir[currentLight] = glm::vec3(0, -1, 0); // sa pointeze in jos
        unitate = glm::rotate(unitate, angleX[currentLight], glm::vec3(1, 0, 0));
        unitate = glm::rotate(unitate, angleZ[currentLight], glm::vec3(0, 0, 1));
        point_light_dir[currentLight] = glm::vec3(unitate * glm::vec4(point_light_dir[currentLight], 0));
    }
}
...
void Lab8::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_F)
    {
        if (isSpotty[currentLight] == 1) {
            isSpotty[currentLight] = 0;
        }
        else {
            isSpotty[currentLight] = 1;
        }
    }
    if (key == GLFW_KEY_T) {
        if (currentLight == 0) {
