...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    // implemented, and the local projection matrix.
    {
        ...
        modelMatrix = glm::scale(modelMatrix, glm::vec3(1.5f, 1.5f, 1.f));
    ...
    }
    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(60.f), glm::vec3(2, 0, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (orto) {
            projectionMatrix = glm::ortho(-5.f, 5.f, -5.f, 5.f, 0.1f, 200.f);
        ...
        if (window->KeyHold(GLFW_KEY_C)) {
            bottom -= deltaTime;
            top += deltaTime;
            projectionMatrix = glm::ortho(left1, right1, bottom, top, 0.01f, 200.0f);
        }
